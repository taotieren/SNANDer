...
int ch347_i2c_write(struct i2c_msg* msg)
{
    ...
    do {
        ...
        *outptr++ = mch347_CMD_I2C_STM_END;
        ...
        ret = libusb_bulk_transfer(Handle, BULK_WRITE_ENDPOINT, i2c_dev.obuf, outptr - i2c_dev.obuf, &actuallen, DEFAULT_TIMEOUT);

        if (ret < 0) {
            fprintf(stderr, "Failed to write to I2C: '%s'\r\n", strerror(-ret));
            return -1;
        }
        ret = libusb_bulk_transfer(Handle, BULK_READ_ENDPOINT, i2c_dev.ibuf, 512, &actuallen, DEFAULT_TIMEOUT);
...
#elif defined(__linux__)
        int ret = libusb_bulk_transfer(Handle, BULK_WRITE_ENDPOINT, i2c_dev.obuf, ptr - i2c_dev.obuf, &actuallen, DEFAULT_TIMEOUT);
        if (ret < 0) {
...
#elif defined(__linux__)
    libusb_bulk_transfer(Handle, BULK_WRITE_ENDPOINT, i2c_dev.obuf, 3, &actuallen, DEFAULT_TIMEOUT);
#endif
    return 0;
}
